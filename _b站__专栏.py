...
class Bilibili:
    ...
    def __divideBilibiliArticle(self, information: dict, author: str,
                                tags: list, url_article: str, url: str,
                                caption: str):
        res = getResModel(type='图片',
                          ...
                          introduction=self.__getValueFromVideo(
                              information, 'desc'),
                          tags=tags,
                          caption=caption,
                          url_article=url_article,
                          url_pic=url,
                          url_local=bilibili_article_picture_path +
        ...
        for record in data['data']['result']:
            if record['result_type'] == 'video':
                for video in record['data']:
                    ...
                    download_video(video['arcurl'], bilibili_video_path,
                                   self.keyword + str(excel.nrows))
                    self.__recordData(temp)

    def __analysisArticle(self, information: dict):

        url_article = "https://www.bilibili.com/read/cv" + str(
            information['id'])
        ...
        for i in img_attr:
            ...
            if caption is None:
                caption = "-"
            # print(caption)
            temp = self.__divideBilibiliArticle(information, author, tags,
                                                url_article, url, caption)
            # print(temp)
            self.__recordData(temp)
    ...
    def __searchBilibiliArticle(self, page: int):
        ...
        # url = "https://api.bilibili.com/x/web-interface/wbi/search/all/v2?page={}&page_size=42&keyword={}"
        response = self.__request(url.format(page, self.keyword))
        data = json.loads(response)
