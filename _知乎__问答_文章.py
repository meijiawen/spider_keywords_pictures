...
class Zhihu:
    ...
    def searchArticle(self, scrollTime: int):
        self.driver.get(
            'https://www.zhihu.com/search?q={}&type=content&utm_id=0&vertical=article'
            .format(self.keyword))
        ...
        urls = []
        introductions = []
        for article in self.driver.find_elements(By.CLASS_NAME,
                                                 "ContentItem.ArticleItem"):
            title = article.find_element(By.CSS_SELECTOR, '[target="_blank"]')
            url = title.get_attribute('href')
            urls.append(url)
            introduction = article.find_element(
                By.CSS_SELECTOR, '[itemprop="articleBody"]').text
    ...
    def searchAnswer(self, scrollTime: int):
        self.driver.get(
            'https://www.zhihu.com/search?q={}&type=content&utm_id=0&vertical=answer'
            .format(self.keyword))
        self.driver.maximize_window()
        ...
        # number = 0
        urls = []
        for answer in self.driver.find_elements(By.CLASS_NAME,
    ...
    def __openArticle(self, url: str, introduction: str):
        ...
        author = self.driver.find_element(
            By.CLASS_NAME,
            "UserLink.AuthorInfo-name").find_element(By.CLASS_NAME,
                                                     'UserLink-link').text

        pics_attrs = self.driver.find_element(
            By.CLASS_NAME,
            "Post-RichTextContainer").find_elements(By.TAG_NAME, "img")
        for pic_attr in pics_attrs:
            pic_url = pic_attr.get_attribute("data-original")
            if pic_url is None:
                continue
            temp = self.__divideZhihu(source="知乎文章",
                                      author=author,
                                      title=title,
                                      introduction=introduction,
                                      url_article=url,
                                      url_pic=pic_url,
                                      url_local=zhihu_article_path +
                                      self.keyword + str(excel.nrows) + ".png")
            print(temp)
    ...
    def __openQuestion(self, url: str, scrollTime: int):
        ...
        title = self.driver.find_element(By.CLASS_NAME,
                                         "QuestionHeader-title").text
        introduction = self.driver.find_element(
            By.CLASS_NAME, "RichText.ztext.css-1g0fqss").text

        answers = self.driver.find_elements(By.CLASS_NAME,
                                            "ContentItem.AnswerItem")
        for answer in answers:
            author = answer.find_element(
                By.CLASS_NAME, "AuthorInfo").find_element(
                    By.CSS_SELECTOR,
                    '[itemprop="name"]').get_attribute("content")
            pics_attrs = answer.find_element(
                By.CLASS_NAME,
                "RichContent-inner").find_elements(By.TAG_NAME, "img")
            for pic_attr in pics_attrs:
                pic_url = pic_attr.get_attribute("data-original")
                if pic_url is None:
                    continue
                temp = self.__divideZhihu(source="知乎问答",
                                          author=author,
                                          title=title,
                                          introduction=introduction,
                                          url_article=url,
                                          url_pic=pic_url,
                                          url_local=zhihu_answer_path +
                                          self.keyword + str(excel.nrows) +
                                          ".png")
                print(temp)
                download_pic(pic_url, temp[record_url_local])
                excel.appendRecord(temp)

    def __divideZhihu(self, source: str, author: str, title: str,
                      introduction: str, url_article: str, url_pic: str,
                      url_local: str):
